...
export default function SignScreen({ doc, mySig, bio = false, signerName = 'ຜູ້ລົງນາມ', meId, onSaveSig, onDone, onBack }) {
  const [stage, setStage] = useState('doc') // doc | choose | draw | connecting | otp | place | bio | success
  const [sig, setSig] = useState(null)
  const [sigType, setSigType] = useState('img') // 'img' | 'lanit'
  ...
  const useLanit = () => { setStage('connecting'); setTimeout(() => { setResendIn(60); setStage('otp') }, 1400) }
  const resendOtp = () => { if (resendIn <= 0) { setResendIn(60); setOtp(['', '', '', '', '', '']); otpRefs.current[0]?.focus() } }
  const flash = (m) => { setToast(m); setTimeout(() => setToast(''), 1800) }
  const otpFull = otp.every((d) => d !== '')
  const submitOtp = () => { if (otpFull) startPlace(null, 'lanit') }
    ...
    <div className={`sign-doc ${gesturing ? 'gesturing' : ''}`} ref={docRef}
      ...
      {hasRealFiles ? (
        <PdfViewer files={files} mode="preview" watermark={!(final && willBeDone)}
          ...
          pageFooter={{ date: (() => { const s = doc.signers.filter((x) => x.status === 'signed' && x.time); return s.length ? s[s.length - 1].time : doc.date })() }}
    ...
    return (
      <div className="app">
        ...
        <div className="scroll sign-scroll">
          <div className="sign-ok"><span className="sign-ok-ic"><Icon.check /></span><div className="sign-ok-txt"><b>{successMsg}</b><p>ລາຍເຊັນຂອງທ່ານໄດ້ຖືກບັນທຶກໃສ່ເອກະສານແລ້ວ</p></div></div>
          {Pages({ withSig: true, final: true })}
        </div>
        <div className="footer"><button className="btn primary" onClick={() => onDone(doc.id, buildSigData())}><Icon.check /> ກັບໜ້າຫຼັກ</button></div>
    ...
    <div className="app">
      ...
      {stage === 'otp' && (
        <div className="modal-overlay">
          <div className="modal-sheet" onClick={(e) => e.stopPropagation()}>
            <div className="modal-head"><b><Icon.shield /> ຢືນຢັນຕົວຕົນ LANIT</b><button className="icon-mini" onClick={() => setStage('choose')}><Icon.x /></button></div>
            <div style={{ padding: '6px 18px 20px', textAlign: 'center' }}>
              <span className="otp-ic"><Icon.shield /></span>
              <p className="muted" style={{ fontSize: 12.5, margin: '6px 0 0' }}>ປ້ອນລະຫັດ OTP 6 ຫຼັກ ທີ່ສົ່ງໄປຫາເບີໂທ/ອີເມວທີ່ລົງທະບຽນກັບ LANIT CA</p>
